...
func generateResourceFromModel(structName string, fields []StructField, authCfg *AuthConfig) string {
	...
	wrapHandler := func(handler string, requireAuth bool) string {
		if requireAuth {
			return fmt.Sprintf("wrapWithAuth(res.%s, pluginRegistry)", handler)
		}
...
`
	...
	if needsAuth {
		authHelperFunc = `
// wrapWithAuth wraps a handler with the auth plugin if available
func wrapWithAuth(handler fiber.Handler, registry *plugin.PluginRegistry) fiber.Handler {
	if authPlugin, ok := registry.GetRoutePlugin("auth"); ok {
		return authPlugin.Wrap(handler)
	}
	return handler
}
`
