...
func TestGenerateStructs(t *testing.T) {
	...
	userFile := filepath.Join(projectRoot, "internal/models", "user.go")
	if _, err := os.Stat(userFile); os.IsNotExist(err) {
		t.Errorf("Expected user.go to be generated, but file does not exist")
	}

	todoFile := filepath.Join(projectRoot, "internal/models", "todo.go")
...
func TestGenerateOpenAPI(t *testing.T) {
	...
	openapiFile := filepath.Join(projectRoot, "internal/openapi", "openapi_gen.go")
