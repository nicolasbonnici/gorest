...
func TestGenerateStructs(t *testing.T) {
	...
	usersFile := filepath.Join(projectRoot, "internal/api/models", "users.go")
	if _, err := os.Stat(usersFile); os.IsNotExist(err) {
		t.Errorf("Expected users.go to be generated, but file does not exist")
	}

	todoFile := filepath.Join(projectRoot, "internal/api/models", "todo.go")
...
func TestGenerateOpenAPI(t *testing.T) {
	...
	openapiFile := filepath.Join(projectRoot, "internal/api/openapi", "openapi_gen.go")
